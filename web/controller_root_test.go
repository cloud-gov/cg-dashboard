...
import (
	...
	. "github.com/18F/cg-dashboard/testhelpers/docker"
)

func TestPing(t *testing.T) {
	response, request := NewTestRequest("GET", "/ping", nil)
	envVars := GetMockCompleteEnvVars()
	cleanup := CreateDefaultV2InfoEndpoint(envVars)
	defer cleanup()
	env, _ := cfenv.Current()
	...
	assert.JSONEq(t, response.Body.String(), expectedResponse)
...
func TestLogout(t *testing.T) {
	for _, test := range logoutTests {
		t.Run(test.TestName, func(t *testing.T) {
			...
			router, store, _ := CreateRouterWithMockSession(t, test.SessionData, test.EnvVars, "")
			router.ServeHTTP(response, request)
