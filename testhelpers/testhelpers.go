...
import (
	...
	app "github.com/18F/cg-dashboard/app"
...
// MockSessionService is the mock service used by the mock session store
type MockSessionService struct{}

// HealthCheck returns true
func (s MockSessionService) HealthCheck() bool { return true }

// Type returns mock for the type of service
func (s MockSessionService) Type() string { return "mock" }
...
func (store MockSessionStore) GetBackendService() services.Service {
...
func CreateDefaultV2InfoEndpoint(envVars app.EnvVars) func() {
	return CreateV2InfoEndpoint(envVars, "")
}

func CreateV2InfoEndpoint(envVars app.EnvVars, tokenEndpoint string) func() {
	gock.New(envVars.APIURL).
		Get("/v2/info").
		Reply(200).
		JSON(cfclient.Endpoint{
			AuthEndpoint:  "https://loginurl",
			TokenEndpoint: tokenEndpoint,
		})
	return gock.Off
}

// func CreateMockSettings(t *testing.T, envVars app.EnvVars, tokenEndpoint string) *helpers.Settings {
// 	settings := &helpers.Settings{}
// 	env, _ := cfenv.Current()
// 	cleanup := CreateV2InfoEndpoint(envVars, tokenEndpoint)
// 	defer cleanup()
// 	err := settings.InitSettings(envVars, env)
// 	if err != nil {
// 		t.Fatalf("Unable to create test settings. Error: %s", err)
// 	}
// 	return settings
// }
...
func CreateRouterWithMockSession(t *testing.T, sessionData map[string]interface{}, envVars app.EnvVars, tokenEndpoint string) (*web.Router, *MockSessionStore, *helpers.Settings) {
	// Initialize settings.
	settings := CreateMockSettings(t, envVars, tokenEndpoint)
...
func GetMockCompleteEnvVars() app.EnvVars {
	return app.EnvVars{
		ClientID:      "ID",
		ClientSecret:  "Secret",
...
func PrepareExternalServerCall(t *testing.T, c *controllers.SecureContext, testServer *httptest.Server, fullURL string, test BasicProxyTest) (*httptest.ResponseRecorder, *http.Request, *web.Router) {
	token, ok := test.SessionData["token"].(oauth2.Token)
	if !ok {
		t.Fatal("Cannot get token data")
		return nil, nil, nil
	}
	// Assign token
	c.Token = token
	...
	router, _, settings := CreateRouterWithMockSession(t, test.SessionData, test.EnvVars, testServer.URL)
